...
function ThemeToggle() {
  ...
  return (
    <button
      ...
      className="relative flex items-center gap-1.5 h-8 rounded-lg px-3 text-xs font-medium text-[--muted-light] hover:text-[--foreground] hover:bg-[--surface] transition-all cursor-pointer select-none"
      title={`PDF theme: ${pdfTheme}`}
    >
      {isDark ? <MoonIcon /> : <SunIcon />}
      <span className="hidden sm:inline">{isDark ? "Dark" : "Light"} PDF</span>
    </button>
  );
}
